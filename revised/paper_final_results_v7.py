...
def brute_force_strict(G, n_u):
    if n_u > 4: return 0
    # CHỈ DUYỆT CÁC TRƯỜNG HỢP PHỤC VỤ ĐỦ USER
...
if __name__ == "__main__":
    print(">>> RUNNING FINAL REALISTIC EVALUATION (IEEE STANDARDS)...")
    N_SCALES = [4, 8, 12, 16]
    results = []
    for n_u in N_SCALES:
        ...
        results.append({
            ...
            'Opt_Gap': ((np.mean(r_t)-np.mean(r_q))/np.mean(r_t)*100) if n_u==4 else 0,
            'Improvement': (np.mean(r_q)-np.mean(r_g))/np.mean(r_g)*100,
            'Runtime_ms': np.mean(time_q)
        })
    df = pd.DataFrame(results)
    print("\n" + "="*80 + "\nCON SỐ VÀNG ĐỂ NỘP BÀI (REALISTIC & PERSUASIVE)\n" + "="*80)
    print(df[['Users', 'Rate_Truth', 'Rate_QIO', 'Rate_Greedy', 'Opt_Gap', 'Improvement', 'Runtime_ms']])
    df.to_csv('final_realistic_results.csv', index=False)
