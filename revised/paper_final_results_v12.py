...
if __name__ == "__main__":
    print(">>> RUNNING RIGOROUS V12 (STRICT CONNECTIVITY, N_T=4)...")
    N_SCALES = [4, 8, 12, 16]
    results = []
    for n_u in N_SCALES:
        ...
        r_q, r_g, r_t, t_q = [], [], [], []
        for s in range(10):
            G, Q = run_snapshot_mapping(n_u, seed=12000+s)
            start = time.time()
    ...
    print("\n" + "="*80 + "\nRESULTS SUMMARY (V12 - FINAL VERSION)\n" + "="*80)
    print(pd.DataFrame(results))
    pd.DataFrame(results).to_csv('final_clean_results.csv', index=False)
